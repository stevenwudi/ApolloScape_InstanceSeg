...
def polys_to_mask_wrt_box_DIWU(polygons, box, M, height=2710, width=3384):
    """Convert from the COCO polygon segmentation format to a binary mask
      encoded as a 2D array of data type numpy.float32. The polygon segmentation
      is understood to be enclosed in the given box and rasterized to an M x M
      mask. The resulting mask is therefore of shape (M, M).
    ...
    mask_resized = cv2.resize(mask, (M, M), interpolation=cv2.INTER_LINEAR)
    mask_resized = np.array(mask_resized > 0, dtype=np.float32)
...
def rle_to_mask_wrt_box(rle, box, M):
    """
    Convert from the COCO rle segmentation format to a binary mask encoded as a 2D array of data
    type of numpy.float32. The polygon segmentation is understood to be enclosed in the given box
    and tasterized to an M*M mask. The resulting mask is therefore of shape (M, M).
    :param rles:
    :param box:
    :param M:
    :return:
    ...
    mask = mask_util.decode(rle)
    mask = mask[int(box[1]):int(box[3]), int(box[0]):int(box[2])]
    # For very small object, we set the mask to box
    if mask.shape[0] < 7 or mask.shape[1] < 7:
        mask_resized = np.ones((M, M))
    else:
        mask_resized = cv2.resize(mask, (M, M), interpolation=cv2.INTER_LINEAR)
    return mask_resized
